...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<ArgTypes>(translator.PushArgTypes, translator.Get, translator.UpdateArgTypes);
        ...
        int ArgTypes_TypeID = -1;
		int ArgTypes_EnumRef = -1;
        
        public void PushArgTypes(RealStatePtr L, ArgTypes val)
        {
            if (ArgTypes_TypeID == -1)
            {
			    bool is_first;
                ArgTypes_TypeID = getTypeId(L, typeof(ArgTypes), out is_first);
				
				if (ArgTypes_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(ArgTypes));
				    ArgTypes_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, ArgTypes_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, ArgTypes_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for ArgTypes ,value="+val);
            }
			
			LuaAPI.lua_getref(L, ArgTypes_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out ArgTypes val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != ArgTypes_TypeID)
				{
				    throw new Exception("invalid userdata for ArgTypes");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for ArgTypes");
                }
				val = (ArgTypes)e;
            ...
            {
                val = (ArgTypes)objectCasters.GetCaster(typeof(ArgTypes))(L, index, null);
            }
        }
		
        public void UpdateArgTypes(RealStatePtr L, int index, ArgTypes val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for ArgTypes ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(ArgTypes[]))
			{
			    ArgTypes[] array = obj as ArgTypes[];
				translator.PushArgTypes(L, array[index]);
				return true;
			}
            return false;
		}
