    ...
    public class UnityEngineRuleTileWrap 
    {
        ...
        static int _m_RuleMatches(RealStatePtr L)
        {
		    try {
                ...
                if(gen_param_count == 6&& translator.Assignable<UnityEngine.RuleTile.TilingRule>(L, 2)&& translator.Assignable<UnityEngine.Vector3Int>(L, 3)&& translator.Assignable<UnityEngine.Tilemaps.ITilemap>(L, 4)&& LuaTypes.LUA_TNUMBER == LuaAPI.lua_type(L, 5)&& LuaTypes.LUA_TBOOLEAN == LuaAPI.lua_type(L, 6)) 
                {
                    ...
                    bool _mirrorX = LuaAPI.lua_toboolean(L, 6);
                    
                        var gen_ret = gen_to_be_invoked.RuleMatches( _rule, _position, _tilemap, _angle, _mirrorX );
                        LuaAPI.lua_pushboolean(L, gen_ret);
