...
public static class ExampleConfig
{
    ...
    static List<string> exclude = new List<string> {
        ...
        "UnityEngine.Input",//实测不能去掉，这个居然会出现InputManagerEntry这个类的wrap。lua webgl不能用啊，可能要换unity版本。。。 只有这个文件是allen自定义的   1. 下面的黑名单没用啊，IsJoystickPreconfigured,这个很难搞，allen直接禁用Input  2. [userelection]标签的需要吗担心c++裁剪 3. 解决IsJoystickPreconfigured的问题要么挨个试把下面黑名单去掉试试，再把我的白名单安排上（如果实在没法解决，又要input性能的话）
        "UnityEngine.AudioSource",//实测不能去掉，这个有很多方法7，8个吧
